...
Friend Class cmdDifferenceGeometrieElement
    Inherits Button
                    ...
                    For Each layer In layers
                        ...
                        If layer.CanEditData Then
                            ...
                            Do While rowCursor.MoveNext()
                                ...
                                If multipoint IsNot Nothing AndAlso resultat.Dimension <= multipoint.Dimension Then
                                    ...
                                    If GeometryEngine.Instance.Intersects(resultat, multipoint) Then
                                        ...
                                        resultat = GeometryEngine.Instance.Difference(resultat, multipoint)
                                    End If
                                End If

                                'Si la Polyline est définie
                                If polyline IsNot Nothing AndAlso resultat.Dimension <= polyline.Dimension Then
                                    'Vérifier si les géométries s'intersectent
                                    If GeometryEngine.Instance.Intersects(resultat, polyline) Then
                                        'Calculer le résultat de la différence
                                        resultat = GeometryEngine.Instance.Difference(resultat, polyline)
                                    End If
                                End If

                                'Si le Polygon est défini
                                If polygon IsNot Nothing AndAlso resultat.Dimension <= polygon.Dimension Then
                                    'Vérifier si les géométries s'intersectent
                                    If GeometryEngine.Instance.Intersects(resultat, polygon) Then
                                        'Calculer le résultat de la différence
                                        resultat = GeometryEngine.Instance.Difference(resultat, polygon)
                                    End If
                                End If
