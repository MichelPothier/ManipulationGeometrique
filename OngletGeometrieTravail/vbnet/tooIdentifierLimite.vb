...
Public Class tooIdentifierLimite
    Inherits MapTool
    ...
    Protected Overrides Function OnSketchCompleteAsync(geometry As Geometry) As Task(Of Boolean)
        ...
        Dim identifyResult = QueuedTask.Run(
            Function()
                ...
                Dim boundary As Geometry = Nothing          'Contient une limite de géométrie d'un élément.
                ...
                For Each layer In layers
                    ...
                    If pfeatures.ContainsKey(layer) Then
                        ...
                        Do While rowCursor.MoveNext()
                            ...
                            boundary = GeometryEngine.Instance.Boundary(shape)
                            'Ajouter la géométrie dans la liste
                            geometries.Add(boundary)
                        Loop
                    End If
                Next
