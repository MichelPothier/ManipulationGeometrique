...
Friend Class cmdIntersectionGeometrieElement
    Inherits Button
                    ...
                    For Each layer In layers
                        ...
                        If layer.CanEditData Then
                            ...
                            Do While rowCursor.MoveNext()
                                ...
                                If multipoint IsNot Nothing AndAlso geometrie.Dimension <= multipoint.Dimension Then
                                    'Vérifier si les géométries s'intersectent
                                    If GeometryEngine.Instance.Intersects(geometrie, multipoint) Then
                                        'Calculer le résultat de l'intersection
                                        Dim resultatP = GeometryEngine.Instance.Intersection(geometrie, multipoint)
                                        'Vérifier si le type de géométrie du résultat est le même
                                        If resultatP.GeometryType = geometrie.GeometryType And Not resultatP.IsEmpty Then
                                            'Si aucun résultat obtenu
                                            If resultat Is Nothing Then
                                                'Définir le résultat obtenu
                                                resultat = resultatP
                                            Else
                                                'Union entre les résultats
                                                resultat = GeometryEngine.Instance.Union(resultat, resultatP)
                                            End If
                                        End If
                                    End If
                                End If
                                ...
                                If polyline IsNot Nothing AndAlso geometrie.Dimension <= polyline.Dimension Then
                                    'Vérifier si les géométries s'intersectent
                                    If GeometryEngine.Instance.Intersects(geometrie, polyline) Then
                                        'Calculer le résultat de l'intersection
                                        Dim resultatL = GeometryEngine.Instance.Intersection(geometrie, polyline)
                                        'Vérifier si le type de géométrie du résultat est le même
                                        If resultatL.GeometryType = geometrie.GeometryType And Not resultatL.IsEmpty Then
                                            'Si aucun résultat obtenu
                                            If resultat Is Nothing Then
                                                'Définir le résultat obtenu
                                                resultat = resultatL
                                            Else
                                                'Union entre les résultats
                                                resultat = GeometryEngine.Instance.Union(resultat, resultatL)
                                            End If
                                        End If
                                    End If
                                End If
                                ...
                                If polygon IsNot Nothing AndAlso geometrie.Dimension <= polygon.Dimension Then
                                    'Vérifier si les géométries s'intersectent
                                    If GeometryEngine.Instance.Intersects(geometrie, polygon) Then
                                        'Calculer le résultat de l'intersection
                                        Dim resultatS = GeometryEngine.Instance.Intersection(geometrie, polygon)
                                        'Vérifier si le type de géométrie du résultat est le même
                                        If resultatS.GeometryType = geometrie.GeometryType And Not resultatS.IsEmpty Then
                                            'Si aucun résultat obtenu
                                            If resultat Is Nothing Then
                                                'Définir le résultat obtenu
                                                resultat = resultatS
                                            Else
                                                'Union entre les résultats
                                                resultat = GeometryEngine.Instance.Union(resultat, resultatS)
                                            End If
                                        End If
                                    End If
                                End If

                                'Vérifier si un résultat est obtenu
