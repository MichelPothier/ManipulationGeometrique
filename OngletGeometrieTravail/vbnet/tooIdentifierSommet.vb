...
Public Class tooIdentifierSommet
    Inherits MapTool
    ...
    Protected Overrides Function OnSketchCompleteAsync(geometry As Geometry) As Task(Of Boolean)
        ...
        Dim identifyResult = QueuedTask.Run(
            Function()
                ...
                Dim multipoint As Multipoint                'Contient les sommets d'une géométrie d'un élément.
                ...
                If rectangle.Area = 0 Then
                    ...
                    geometry = rectangle.Points.First
                End If
                ...
                For Each layer In layers
                    ...
                    If pfeatures.ContainsKey(layer) Then
                        ...
                        Do While rowCursor.MoveNext()
                            ...
                            If shape.GeometryType = GeometryType.Point Then
                                'Définir la ligne
                                Dim point As MapPoint = shape
                                'Définir le constructeur de Multipoint
                                Dim multipointBuilder As New MultipointBuilder(point)
                                'Définir la référence spatiale
                                multipointBuilder.SpatialReference = shape.SpatialReference
                                'Construire le multipoint
                            ...
                            ElseIf shape.GeometryType = GeometryType.Multipoint Then
                                'Définir la ligne
                                multipoint = shape
                                'Définir le constructeur de Multipoint
                                Dim multipointBuilder As New MultipointBuilder(multipoint)
                                'Définir la référence spatiale
                                multipointBuilder.SpatialReference = shape.SpatialReference
                                'Construire le multipoint
                                multipoint = multipointBuilder.ToGeometry
                            ...
                            ElseIf shape.GeometryType = GeometryType.Polyline Then
                                'Définir la ligne
                                Dim polyline As Polyline = shape
                                'Définir le constructeur de Multipoint
                                Dim multipointBuilder As New MultipointBuilder(polyline)
                                'Définir la référence spatiale
                                multipointBuilder.SpatialReference = shape.SpatialReference
                                'Construire le multipoint
                                multipoint = multipointBuilder.ToGeometry
                            ...
                            ElseIf shape.GeometryType = GeometryType.Polygon Then
                                'Définir la ligne
                                Dim polygone As Polygon = shape
                                'Définir le constructeur de Multipoint
                                Dim multipointBuilder As New MultipointBuilder(polygone)
                                'Définir la référence spatiale
                                multipointBuilder.SpatialReference = shape.SpatialReference
                                'Construire le multipoint
                                multipoint = multipointBuilder.ToGeometry
